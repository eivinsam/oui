                       ...
                       gpc_tristrip *result)
{
  sb_tree       *sbtree= NULL;
  it_node       *it= NULL, *intersect;
  edge_node     *prev_edge, *next_edge, *succ_edge, *e0, *e1;
  ...
  while (scanbeam < sbt_entries)
  {
    ...
    if (local_min)
    {
      ...
      {
        /* Add edges starting at this local minimum to the AET */
        for (edge_node *edge= local_min->first_bound; edge; edge= edge->next_bound)
          add_edge_to_aet(&aet, edge, NULL);
         ...
         && (e1->bundle[ABOVE][CLIP] || e1->bundle[ABOVE][SUBJ]))
	{
      ...
      /* Prepare for next scanbeam */
      for (edge_node *edge= aet; edge; edge= next_edge)
      {
        next_edge= edge->next;
        succ_edge= edge->succ;
